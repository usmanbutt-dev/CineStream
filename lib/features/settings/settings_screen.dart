...
class SettingsScreen extends ConsumerWidget {
              ...
              Wrap(
                ...
                children: _accentPresets.entries.map((entry) {
                  final isSelected = current.accentColor == null
                      ? entry.value == CineColors.primary
        ...
        content: Column(
          ...
          children: [
            Container(
              ...
              child: Text(
                current ?? 'Default (App Documents/CineStream/downloads)',
                style: theme.textTheme.bodySmall?.copyWith(
                  color: CineColors.textSecondary,
                  height: 1.5,
                ),
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          FilledButton(
            style: isDangerous
                ? FilledButton.styleFrom(backgroundColor: CineColors.error)
                : null,
            onPressed: () async {
              Navigator.pop(ctx);
              try {
  ...
  Future<List<String>> _downloadDirs(WidgetRef ref) async {
    ...
    dirs.add(p.join(appDir.path, 'CineStream', 'downloads'));
    return dirs;
  }
  ...
  void _showDisclaimerDialog(BuildContext context, ThemeData theme) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: const Row(
          children: [
            Icon(Icons.gavel_rounded, color: CineColors.warning),
            SizedBox(width: 8),
            Text('Disclaimer'),
          ],
        ),
...
class _SettingsTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListTile(
      leading: Icon(icon, color: CineColors.textSecondary),
